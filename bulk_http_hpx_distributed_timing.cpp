...
auto ns(std::chrono::duration<auto, auto> duration) {
	return std::chrono::nanoseconds{duration}.count();
}

auto ns(std::chrono::time_point<std::chrono::steady_clock> tp) {
	return ns(tp.time_since_epoch());
}
...
execute_function(std::string function_path, std::vector<uint8_t> input) {
...
#ifdef TIMING
	std::array<std::chrono::time_point<std::chrono::steady_clock>, 9> timings;
	...
	std::vector<uint8_t> result;
	{
		...
		// we could allocate only the needed space in this case but it is
		// probably better to keep the determinism
	...
	hpx::util::format_to(
		hpx::cout, "{},{},{},{},{},{},{},{},{},", ns(timings[0]), ns(timings[1]),
		ns(timings[2]), ns(timings[3]), ns(timings[4]), ns(timings[5]),
		ns(timings[6]), ns(timings[7]), ns(timings[8])
	) << std::flush;
#endif
	return result;
}
...
int main(int argc, char* argv[]) {
	try {
		timings.fill(std::chrono::time_point<std::chrono::steady_clock>());
